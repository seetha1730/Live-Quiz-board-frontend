        ...
        {questions ? (
          questions.map((question, index) => (
            <div key={index} className={`flex flex-col p-5 m-5 bg-white rounded-lg text-gray-700 ${sentQuestion.includes(index)? 'grayscale':""}`}>
              ...
              <button  onClick={() => sendQuestion(question, index)} className="relative inline-flex items-center justify-center mt-4 p-4 px-6 py-3 overflow-hidden bg-[#464857]  font-medium text-white transition duration-300 ease-out border-2 border-blue-500 rounded-lg shadow-md group">
